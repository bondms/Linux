...
# The no-DHCP network is not served by DHCP.
# To access the internet, configure the following static IP parameters:
# IPv4 address: 172.17.x.x (e.g. 172.17.1.1)
# netmask: 255.255.0.0
# gateway: 172.17.0.1
...
sudo kvm \
    ...
    -cpu host \
    -m 4096 \
    -net nic,macaddr=${MACADDR},model=virtio -net tap,script=/usr/local/bin/qemu-ifup-nodhcp,downscript=/usr/local/bin/qemu-ifdown-nodhcp \
    -no-quit \
    -runas `whoami` \
